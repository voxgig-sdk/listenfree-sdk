...
class MusicDirectTest < Minitest::Test
  def test_direct_list_music
    setup = music_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-music", setup[:live] ? "live" : "unit")
    if _should_skip
...
def music_direct_setup(mockres)
