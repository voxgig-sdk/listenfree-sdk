...
(0, node_test_1.describe)('SearchDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-search', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-search', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            query.q = "imagine dragons";
        }
        else {
        }
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['LISTENFREE_TEST_SEARCH_ENTID'];
