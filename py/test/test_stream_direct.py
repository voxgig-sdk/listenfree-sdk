...
class TestStreamDirect:
    ...
    def test_should_direct_load_stream(self):
        setup = _stream_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-stream", "live" if setup["live"] else "unit")
        if _skip:
...
def _stream_direct_setup(mockres):
