-- Listenfree SDK Stream entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local StreamEntity = {}
StreamEntity.__index = StreamEntity


function StreamEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StreamEntity)
  self._name = "stream"
  self._client = client
...
function StreamEntity:get_name()
  return self._name
end


function StreamEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StreamEntity.new(self._client, opts)
end
...
function StreamEntity:mark_deleted()
  self._deleted = true
end


function StreamEntity:deleted()
  return true == self._deleted
end


function StreamEntity:data_set(args)
  if args ~= nil then
...
function StreamEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StreamEntity:match_set(args)
  if args ~= nil then
...
function StreamEntity:match_get()
...
function StreamEntity:stream(action, args, callopts)
...
---@param reqmatch StreamLoadMatch
---@param ctrl? table
---@return Stream
---@return string? err
function StreamEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function StreamEntity:_run_op(ctx, post_done)
  local utility = self._utility
