...
describe("ListeningRoomEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:ListeningRoom(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:ListeningRoom(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:ListeningRoom(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = listening_room_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "listening_room." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set LISTENFREE_TEST_LISTENING_ROOM_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local listening_room_ref01_ent = client:ListeningRoom(nil)
    local listening_room_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LIST
    local listening_room_ref01_match = {}

    local listening_room_ref01_list_result, err = listening_room_ref01_ent:list(listening_room_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(listening_room_ref01_list_result)
    ...
    -- LOAD
    local listening_room_ref01_match_dt0 = {
      id = listening_room_ref01_data["id"],
    }
...
function listening_room_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/listening_room/ListeningRoomTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read listening_room test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("LISTENFREE_TEST_LISTENING_ROOM_ENTID")
