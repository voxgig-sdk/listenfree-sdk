...
-- by passing it.
function ListenfreeSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ListenfreeSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ListenfreeSDK: graphql: " .. msg
    res.graphql = errors
  end
