...
class TestMusicDirect:

    def test_should_direct_list_music(self):
        setup = _music_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-music", "live" if setup["live"] else "unit")
        if _skip:
...
def _music_direct_setup(mockres):
