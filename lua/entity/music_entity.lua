-- Listenfree SDK Music entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local MusicEntity = {}
MusicEntity.__index = MusicEntity


function MusicEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MusicEntity)
  self._name = "music"
  self._client = client
...
function MusicEntity:get_name()
  return self._name
end


function MusicEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MusicEntity.new(self._client, opts)
end
...
function MusicEntity:mark_deleted()
  self._deleted = true
end


function MusicEntity:deleted()
  return true == self._deleted
end


function MusicEntity:data_set(args)
  if args ~= nil then
...
function MusicEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MusicEntity:match_set(args)
  if args ~= nil then
...
function MusicEntity:match_get()
...
function MusicEntity:stream(action, args, callopts)
...
---@param reqmatch MusicListMatch
---@param ctrl? table
---@return Music[]
---@return string? err
function MusicEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function MusicEntity:_run_op(ctx, post_done)
  local utility = self._utility
