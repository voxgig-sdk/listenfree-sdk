...
type Args struct {
	Entity string         `json:"entity" jsonschema:"listening_room | music | offline_download | playlist | search | song | stream | video"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.ListenfreeSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "listenfree_list",
		Description: "List records from Listenfree. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "listenfree_load",
		Description: "Load a single record from Listenfree. " +
...
func runOp(client *sdk.ListenfreeSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.ListenfreeSDK, name string) (sdk.ListenfreeEntity, error) {
	switch strings.ToLower(name) {
	case "listening_room":
		return client.ListeningRoom(nil), nil
	case "music":
		return client.Music(nil), nil
	case "offline_download":
		return client.OfflineDownload(nil), nil
	case "playlist":
		return client.Playlist(nil), nil
	case "search":
		return client.Search(nil), nil
	case "song":
		return client.Song(nil), nil
	case "stream":
		return client.Stream(nil), nil
	case "video":
