...
class TestSongDirect:

    def test_should_direct_load_song(self):
        setup = _song_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-song", "live" if setup["live"] else "unit")
        if _skip:
...
def _song_direct_setup(mockres):
