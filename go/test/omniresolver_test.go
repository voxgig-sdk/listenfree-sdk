...
import (
	...
	omni "github.com/voxgig-sdk/listenfree-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ListenfreeError); is {
				out := map[string]any{
					"name":    "ListenfreeError",
...
func omniCtx(arg any, client *sdk.ListenfreeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ListenfreeSDK); is {
			client = live
