...
describe("OfflineDownloadEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:OfflineDownload(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = offline_download_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "offline_download." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set LISTENFREE_TEST_OFFLINE_DOWNLOAD_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local offline_download_ref01_ent = client:OfflineDownload(nil)
    local offline_download_ref01_data = helpers.to_map(vs.getprop(
...
function offline_download_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/offline_download/OfflineDownloadTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read offline_download test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("LISTENFREE_TEST_OFFLINE_DOWNLOAD_ENTID")
