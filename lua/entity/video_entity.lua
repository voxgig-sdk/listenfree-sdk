-- Listenfree SDK Video entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local VideoEntity = {}
VideoEntity.__index = VideoEntity


function VideoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, VideoEntity)
  self._name = "video"
  self._client = client
...
function VideoEntity:get_name()
  return self._name
end


function VideoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return VideoEntity.new(self._client, opts)
end
...
function VideoEntity:mark_deleted()
  self._deleted = true
end


function VideoEntity:deleted()
  return true == self._deleted
end


function VideoEntity:data_set(args)
  if args ~= nil then
...
function VideoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function VideoEntity:match_set(args)
  if args ~= nil then
...
function VideoEntity:match_get()
...
function VideoEntity:stream(action, args, callopts)
...
function VideoEntity:_run_op(ctx, post_done)
  local utility = self._utility
