...
boru-driven command-line client **and** interactive REPL for the Listenfree
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/listenfree-cli)
make build
...
./listenfree-cli list listening_room
./listenfree-cli load 1 listening_room            # {id:1} shorthand
./listenfree-cli load '{id:1}' listening_room       # explicit match map
./listenfree-cli list music

# 5. Override the API base URL for a single call
LISTENFREE_BASE=https://api.example.com ./listenfree-cli list listening_room

# 6. No arguments -> interactive REPL
./listenfree-cli
listenfree> list listening_room
listenfree> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/listenfree-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/listenfree-cli list listening_room
   ```
...
```sh
./listenfree-cli load 1 listening_room          # scalar shorthand for {id:1}
./listenfree-cli load '{id:1}' listening_room     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `listenfree>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `listening_room`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/listenfree-cli`. |
...
(`./listenfree-cli <expr>`) and interactively (the REPL), and expressions compose the
