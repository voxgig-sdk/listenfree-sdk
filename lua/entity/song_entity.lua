-- Listenfree SDK Song entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local SongEntity = {}
SongEntity.__index = SongEntity


function SongEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SongEntity)
  self._name = "song"
  self._client = client
...
function SongEntity:get_name()
  return self._name
end


function SongEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SongEntity.new(self._client, opts)
end
...
function SongEntity:mark_deleted()
  self._deleted = true
end


function SongEntity:deleted()
  return true == self._deleted
end


function SongEntity:data_set(args)
  if args ~= nil then
...
function SongEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SongEntity:match_set(args)
  if args ~= nil then
...
function SongEntity:stream(action, args, callopts)
...
function SongEntity:_run_op(ctx, post_done)
  local utility = self._utility
