...
module ListenfreeUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ListenfreeResponse.new({})
    ...
    if fetch_err
      ...
      response = ListenfreeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ListenfreeResponse.new(fetched)
    else
