...
class PlaylistDirectTest extends TestCase
{
    ...
    public function test_direct_load_playlist(): void
    {
        $setup = playlist_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-playlist", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
