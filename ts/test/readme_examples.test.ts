...
//     (\`new ListenfreeSDK(...)\` / \`ListenfreeSDK.test(...)\` become
//     \`ListenfreeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"listening_room":{"test01":{"id":"test01"}},"music":{"test01":{"id":"test01"}},"offline_download":{"test01":{"id":"test01"}},"playlist":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}},"song":{"test01":{"id":"test01"}},"stream":{"test01":{"id":"test01"}},"video":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ListenfreeSDK(...)\` and any
// \`ListenfreeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ListenfreeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ListenfreeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
