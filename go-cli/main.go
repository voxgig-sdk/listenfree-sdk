// Command Listenfree-cli is a CLI + REPL for the Listenfree SDK,
...
const prompt = "listenfree"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "listening_room music offline_download playlist search song stream video"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      LISTENFREE_APIKEY (api key), LISTENFREE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: LISTENFREE_APIKEY carries the API key and
	// LISTENFREE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("LISTENFREE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("LISTENFREE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewListenfreeSDK(opts)
