...
describe('ListeningRoomDirect', async () => {
  ...
  test('direct-load-listening_room', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-listening_room', setup.live)) return
  ...
  test('direct-list-listening_room', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-listening_room', setup.live)) return
    const { client, calls } = setup
