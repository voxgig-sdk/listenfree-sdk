...
class TestVideoEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Video(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _video_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "video." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set LISTENFREE_TEST_VIDEO_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        video_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.video")))
        video_ref01_data = None
        if len(video_ref01_data_raw) > 0:
        ...
        assert err is None
        assert video_ref01_data_dt0_loaded is not None



def _video_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/video/VideoTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("LISTENFREE_TEST_VIDEO_ENTID"))
