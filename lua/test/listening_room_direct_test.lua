...
describe("ListeningRoomDirect", function()
  it("should direct-list-listening_room", function()
    local setup = listening_room_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-listening_room", setup.live and "live" or "unit")
  ...
  it("should direct-load-listening_room", function()
    local setup = listening_room_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-listening_room", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function listening_room_direct_setup(mockres)
