...
func TestVideoEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		videoRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.video", setup.data)))
		var videoRef01Data map[string]any
		if len(videoRef01DataRaw) > 0 {
			videoRef01Data = core.ToMapAny(videoRef01DataRaw[0][1])
		}
...
func videoBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"video01", "video02", "video03", "song01", "song02", "song03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("LISTENFREE_TEST_VIDEO_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LISTENFREE_TEST_VIDEO_ENTID"])
