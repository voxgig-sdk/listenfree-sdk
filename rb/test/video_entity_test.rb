...
class VideoEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set LISTENFREE_TEST_VIDEO_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    video_ref01_data = nil
    if video_ref01_data_raw.length > 0
...
def video_basic_setup(extra)
  ...
  entid_env_raw = ENV["LISTENFREE_TEST_VIDEO_ENTID"]
