...
class FtCtrl extends ListenfreeControl
{
...
class FtClient
{
    ...
    private ListenfreeUtility $utility;

    public function __construct(ListenfreeUtility $utility, array $options)
    {
    ...
    public function get_utility(): ListenfreeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ListenfreeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ListenfreeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ListenfreeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ListenfreeConfig::make_feature($name);
    ...
    private static function buildUrl(ListenfreeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ListenfreeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ListenfreeResponse($fetched);
        }
    ...
    private function fail(ListenfreeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ListenfreeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
