...
The Python SDK for the Listenfree API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.ListeningRoom(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.ListeningRoom(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
```python
# Create
created, _ = client.ListeningRoom(None).create({"name": "Example"}, None)
...
result, err = client.Listenfree(None).load(
    {"id": "test01"}, None
)
