...
class SongDirectTest < Minitest::Test
  def test_direct_load_song
    setup = song_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-song", setup[:live] ? "live" : "unit")
    if _should_skip
