...
describe("PlaylistEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Playlist(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Playlist(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Playlist(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set LISTENFREE_TEST_PLAYLIST_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local playlist_ref01_ent = client:Playlist(nil)
    local playlist_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- UPDATE
    local playlist_ref01_data_up0_up = {
      id = playlist_ref01_data["id"],
    }
    ...
    -- REMOVE
    local playlist_ref01_match_rm0 = {
      id = playlist_ref01_data["id"],
    }
    local _, err = playlist_ref01_ent:remove(playlist_ref01_match_rm0, nil)
    assert.is_nil(err)

    -- LIST
    local playlist_ref01_match_rt0 = {}

    local playlist_ref01_list_rt0_result, err = playlist_ref01_ent:list(playlist_ref01_match_rt0, nil)
    assert.is_nil(err)
    assert.is_table(playlist_ref01_list_rt0_result)
...
function playlist_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/playlist/PlaylistTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read playlist test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("LISTENFREE_TEST_PLAYLIST_ENTID")
