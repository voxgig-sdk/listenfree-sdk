...
class ListeningRoomEntity
  ...
  def make
    opts = @_entopts.dup
    ListeningRoomEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.ListeningRoom.load works with no args).
  ...
  #
  # @param reqmatch [ListeningRoomListMatch, Hash, nil] match filter (any subset of
  #   ListeningRoom fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
