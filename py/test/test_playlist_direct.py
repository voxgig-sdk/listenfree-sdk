...
class TestPlaylistDirect:
    ...
    def test_should_direct_load_playlist(self):
        setup = _playlist_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-playlist", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
