...
class SearchEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LISTENFREE_TEST_SEARCH_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        [$search_ref01_data_dt0_loaded, $err] = $search_ref01_ent->load($search_ref01_match_dt0, null);
        $this->assertNull($err);
...
function search_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("LISTENFREE_TEST_SEARCH_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["LISTENFREE_TEST_SEARCH_ENTID"]);
