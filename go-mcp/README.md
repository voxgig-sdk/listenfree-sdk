...
[MCP](https://modelcontextprotocol.io) server exposing the Listenfree SDK as
two agent tools — `listenfree_list` and `listenfree_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/listenfree-mcp)
make build

# 2. Provide credentials via the environment
export LISTENFREE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user listenfree \
  -- /absolute/path/to/listenfree-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./listenfree-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// listenfree_list: first page of records
{ "entity": "listening_room" }
{ "entity": "listening_room", "query": { } }

// listenfree_load: one record by id
{ "entity": "listening_room", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/listenfree-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user listenfree \
     -- "$PWD"/dist/*/listenfree-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `listenfree_list` and `listenfree_load` tools now appear
   in new sessions. Ask the agent to *"list listening_room using listenfree"*
   and it calls `listenfree_list` with `{"entity":"listening_room"}`.
...
```sh
export LISTENFREE_APIKEY=sk_live_xxx            # API key
export LISTENFREE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `listenfree_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `listenfree_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `LISTENFREE_APIKEY` | API key sent with every request. |
| `LISTENFREE_BASE` | Optional override of the API base URL. |

### Entities

The 8 entities valid as the `entity` argument:

listening_room | music | offline_download | playlist | search | song | stream | video

### Smoke test via HTTP (raw JSON-RPC)

```sh
./listenfree-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"listenfree_load","arguments":{"entity":"listening_room","query":{"id":1}}}}'
```
