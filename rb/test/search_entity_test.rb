...
class SearchEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set LISTENFREE_TEST_SEARCH_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    search_ref01_data_dt0_loaded, err = search_ref01_ent.load(search_ref01_match_dt0, nil)
    assert_nil err
...
def search_basic_setup(extra)
  ...
  entid_env_raw = ENV["LISTENFREE_TEST_SEARCH_ENTID"]
