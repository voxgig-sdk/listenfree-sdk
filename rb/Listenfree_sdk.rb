...
class ListenfreeSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ListenfreeHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ListenfreeHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ListenfreeFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ListenfreeUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = ListenfreeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ListenfreeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ListenfreeHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ListenfreeError.new(
        "graphql_error", "ListenfreeSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
